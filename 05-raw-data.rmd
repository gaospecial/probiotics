...
经过调研，最终决定依托 'The All-Species Living Tree' Project （LTP）来完成。
...
ltp <- read.newick(file = "data-raw/LTPs132_SSU_tree.newick")
```
...
```{r}
library(tidyr)
organism <- tibble(ltp = ltp$tip.label) %>%
  separate(col = ltp, 
           into = c("accession","genus_species","family","extra"),
           sep = "_,") %>%
